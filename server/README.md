...
Use the preconfigured LLM helpers. Set `OPENAI_API_KEY` on the server, and optionally `OPENAI_MODEL` if you want to override the default `gpt-5-mini`.
...
| `OPENAI_API_KEY` | OpenAI API key used by the server-side LLM adapter |
